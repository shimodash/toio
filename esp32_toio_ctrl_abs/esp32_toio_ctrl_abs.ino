...
static BLERemoteCharacteristic* pRemoteCharacteristic[ NUM_OF_MAX_CUBE ][ 13 ];
...
// state
enum { CUBE_FWD, CUBE_STOP, CUBE_REV, CUBE_CCW, CUBE_CW ,CUBE_0 ,CUBE_1 ,CUBE_2} Cube_state[ NUM_OF_MAX_CUBE ];
...
//Motor Set Value
int MotorSetValue_pos ( int n, int x, int y,  int angle){
 byte comm[] = {03,00,0x10,0x00,0x50,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}; 
 
 comm[7] = (byte)(lowByte(x));
 comm[8] = (byte)(highByte(x));
 comm[9] = (byte)(lowByte(y));
 comm[10] = (byte)(highByte(y));
 comm[11] = (byte)(lowByte(angle));  
 comm[12] = (byte)(highByte(angle));  
 ...
 comm[12] = (byte)(0x0);  
*/
 Serial.println(String(highByte(x)));
 Serial.println(String(lowByte(x)));

 pRemoteCharacteristic[n][MOTOR_CONT]->writeValue(comm, 13);
 return (SUCCESSFUL_COMPLETION);
}
...
#define LEFT_ID    8 
#define RIGHT_ID  46
#define CARD0_ID  939524144
#define CARD1_ID  939524145
#define CARD2_ID  939524146
...
void event_detect(){
 ...
 if( curID >= 0){
   ...
   switch( curID ){
     ...
     case CARD0_ID:
       SoundSetValue( cube_no,30,100,127 );
       MotorSetValue_pos( tg_cube,147,214, 0); 
       Cube_state[tg_cube] = CUBE_0;
       DisplayWrite("CUBE_0");
       break;                
     case CARD1_ID:
       SoundSetValue( cube_no,30,100,127 );
       MotorSetValue_pos( tg_cube,177,214, 90); 
       Cube_state[tg_cube] = CUBE_0;
       DisplayWrite("CUBE_1");
       break;                
     case CARD2_ID:
       SoundSetValue( cube_no,30,100,127 );
       MotorSetValue_pos( tg_cube,177,244, 180); 
       Cube_state[tg_cube] = CUBE_0;
       DisplayWrite("CUBE_2");
       break;                
   }
