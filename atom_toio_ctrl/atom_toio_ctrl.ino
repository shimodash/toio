...
// キャラクタリスティックを格納する配列のINDEX ------------- 
 ...
 #define NUM_OF_MAX_CUBE 1 // CUBEの最大接続数
...
int sp_left,sp_right;
 ...
 if(FSM==0){
  ...
  sp_left  = cube_speed[tg_cube]  + ((curAng2 - 2 )*3 );
  sp_right = cube_speed[tg_cube]  - ((curAng2 - 2 )*3 );
 ...
 if (M5.Btn.wasPressed()){
       switch (FSM)
        {
        case 0:
            SoundSetValue( tg_cube,30,100,127 );
            break;
        case 1:
            SoundSetValue( tg_cube,30,100,127 );
            break;
...
//        Serial.printf("%.2f,%.2f,%.2f mg\r\n", accX * 1000, accY * 1000, accZ * 1000);
        curAng2=constrain(accX * 5 + 2, 0, 4);
